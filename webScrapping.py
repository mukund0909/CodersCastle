...
class UserData:
    ...
    def __atcoder(self):
        ...
        span1=rows[0].find_all("span")
        span2=rows[1].find_all("span")
        try:
            current_rating = int(span1[0].text)
            highest_rating = int(span2[0].text)
            level=span2[2].text
        except Exception as E:
            raise BrokenChangesError(E)
        rating_container = soup.find('div', class_='mt-2 mb-2')
        rating_list=rating_container.find_all('script')
        #print(rating_list[1].text)
        ratings=rating_list[1].text
        rating_string=""
        flag=0
        for ch in ratings:
            if ch=='[':
                flag=1
            if flag==1:
                rating_string=rating_string+ch;
            if ch==']':
                flag=0
        jsonObject=json.loads(rating_string)
        rating_list=[]
        for obj in jsonObject:
            rating_list.append(obj.get('NewRating'))
        details = {
            "status": "Success",
            "username": self.__username,
            "platform": "Atcoder",
